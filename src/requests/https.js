 ...
 * 设置超时时间和跨域是否允许携带凭证
 */
api.defaults.timeout=10000  //设置十秒
api.defaults.withCredentials=true ;//例如：登录校验session和cookie
/**
 * 设置请求数据参数传递的格式，默认是json格式，但是在登录校验中后台一般设置请求格式：x-www-form-urlencoded(name=xxx,age=xxx)
 * 看服务器要求什么格式
 */
api.defaults.headers['Content-Type']='application/x-www-form-urlencoded' //声明请求格式
api.defaults.transformRequest=data=>qs.stringify(data)  //qs是第三方库，转换为x-www-form-urlencoded
/**
